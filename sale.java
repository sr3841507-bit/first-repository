...
class sale extends JInternalFrame implements ActionListener,FocusListener,ListSelectionListener
{
Connection con;
...
l11=new JLabel("TOTAL AMT:");



l1.setFont(font1);
l1.setForeground(Color.decode("#825D55"));
l1.setBounds(800,20,800,40);
...
if(qty>qtavail)
JOptionPane.showMessageDialog(this, "ERROR , Quantity not Available!");
 else
{
...
ResultSet rs = stmt.executeQuery("SELECT qty, amt FROM sbilltrans WHERE billno='" + billno + "' AND iname='" + iname + "'");
if(rs.next()) {
...
try{
 ...
 if(e.getSource() == b1) {
Statement stmt = con.createStatement();
ResultSet rs = stmt.executeQuery("Select billno from sbillmaster");
count1=1;
int a=1;
while(rs.next()) {
a++;
...
{
Statement stmt = con.createStatement();
ResultSet rs = stmt.executeQuery("Select clbalance from sparty where name='"+c3.getSelectedItem().toString()+"'");
...
Statement stmt1=con.createStatement();
ResultSet rs2=stmt1.executeQuery("Select totalamount from sbillmaster where partyname='"+c3.getSelectedItem().toString()+"'");
double tot=0;
while(rs2.next())
{
...
{
String sq4 = "insert into sbilltrans values('"+t1.getText()+ "', '" + c4.getSelectedItem()+ "', '" +t4.getText()+ "', '" +t5.getText()+ "', '" + t7.getText()+ "')";
...
if(count1==1)
{
String sq1 ="insert into sbillmaster values('" + billno + "', '"+ partyname +"', '"+billdate +"', '" + totalamount +"')";
stmt.executeUpdate(sq1);
}
else if(count1==2)
{
String sq1 = "update sbillmaster set totalamount='"+t8.getText()+"' where billno= '"+t1.getText()+"'";
stmt.executeUpdate(sq1);

String sq3 = "update sparty set clbalance='"+t3.getText()+"' where name= '"+c3.getSelectedItem()+"'";
...
stmt.executeUpdate("delete from sbilltrans where billno='" + billno + "' AND iname='" + iname + "'");
ResultSet rs5 = stmt.executeQuery("select totalamount from sbillmaster where billno='" + billno + "'");

if(rs5.next())
 {
double totalAmt = Double.parseDouble(rs5.getString("totalamount"));
double updatedTotal = totalAmt - amt;
stmt.executeUpdate("update sbillmaster set totalamount='" + updatedTotal + "' where billno='" + billno + "'");
...
String partyname = c3.getSelectedItem().toString();
ResultSet r6 = stmt.executeQuery("Select clbalance from sparty where name='" + partyname + "'");
double opbal = 0;
if(r6.next()) 
{
...
ResultSet rs7= stmt.executeQuery("Select totalamount from sbillmaster where partyname='" + partyname + "'");
double totalAmt = 0;
while(rs7.next()) 
{
...
t7.setText("");
Statement stmt=con.createStatement();
String s2="select * from sbillmaster where billno='"+t9.getText()+"'";
...
Statement stmt2=con.createStatement();
String s3="select * from sbilltrans where billno='"+t9.getText()+"'";
