...
class payment extends JInternalFrame implements ActionListener
{
Connection con;
...
ResultSet rs = stmt.executeQuery("Select serial from payment");
count1=1;
int a=1;
while(rs.next()) {
a++;
...
{
Statement stmt = con.createStatement();
ResultSet rs = stmt.executeQuery("Select opbalance from pparty where name='"+c2.getSelectedItem().toString()+"'");
...
String pdate = t3.getText();
String amt = t5.getText();
String details = t6.getText();

String sq1 ="insert into payment values('" +serial + "', '"+ pname +"', '"+pdate +"', '" + amt +"','"+details+"')";
...
{
      Statement stmt = con.createStatement();
    String s = "update payment set amt='" + t5.getText() + "', details='" + t6.getText() +"'where pname='"+c2.getSelectedItem()+"'";
...
Statement stmt=con.createStatement();
String s2="select * from payment where serial='"+t7.getText()+"'";
