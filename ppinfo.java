...
class ppinfo extends JInternalFrame implements ActionListener
{
Connection con;
...
l1=new JLabel("PURCHASE PARTY BALANCE INFORMATION");
...
ResultSet rs = stmt.executeQuery("Select address,phone,opbalance from pparty where name='"+c1.getSelectedItem().toString()+"'");
if(rs.next())
{
...
ResultSet rs3=stmt2.executeQuery("Select amt from payment where pname='"+c1.getSelectedItem().toString()+"'");
double total_pay=0;
while(rs3.next())
{
total_pay=total_pay+Double.parseDouble(rs3.getString("amt"));
}


Statement stmt1=con.createStatement();
ResultSet rs2=stmt1.executeQuery("Select totalamount from pbillmaster where partyname='"+c1.getSelectedItem().toString()+"'");
double total_pur=0;
while(rs2.next())
{
total_pur=total_pur +Double.parseDouble(rs2.getString("totalamount"));
}
double final_amt=opbal+total_pur-total_pay;
